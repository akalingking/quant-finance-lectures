...
def zoom(f, x, beta_l, beta_h, c1=10**-1, c2=0.9, verbose=False):
	...
	while j<N:
		...
		if f(x + beta*delta) > f(x) + c1*beta*np.dot(T(gf(x)), delta)\
			...
			if (beta_r - beta_l) * np.dot(T(gf(x + beta_l*delta)),delta) >= 0.0:
				beta_h = beta_l 
		j += 1
	if verbose: print(f"\tzoom [{j}] beta*: {beta_star}")
	return beta_star

def line_search(f, x, beta_l=0.0, beta_h=1.0, c1=10**-1, c2=0.9, maxIter=20, epsilon=10**-6, verbose=False):
	"""
	...
	while j<N:
		...
		if f(x + beta*delta) > f(x) + c1 * beta* np.dot(T(gf(x)),delta)\
			...
			# Check for Armijo condition with parameter c1 set to 10^-4 for sufficient decrease
...
def test():
	...
	delta = -gf(x)/Norm(gf(x))
	print(f"Custom implemention  \tbeta:{beta:.6f} f(x):{f(x):.6f} < f(x+beta*delta):{f(x + beta*delta):.6f}")
	beta_exp = sp.optimize.line_search(f, myfprime=gf, xk=x, pk=delta)[0]
	print(f"Scipy implementation \tbeta_exp:{beta_exp} f(x):{f(x):.6f} < f(x+beta*delta): {f(x+beta_exp*delta):.6f}")
